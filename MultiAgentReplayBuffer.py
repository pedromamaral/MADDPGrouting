...
class MultiAgentReplayBuffer:
    ...
    def init_actor_memory(self):
        ...
        for i in range(self.n_agents):
            ...
            self.state_memory.append(np.zeros((self.mem_size, self.crict_dims[i])))
            self.new_state_memory.append(np.zeros((self.mem_size, self.crict_dims[i])))

    def store_transition(self, raw_obs, action, reward,
                         raw_obs_, done, critic_sates, critic_new_states):
        ...
        for agent_idx in range(self.n_agents):
            ...
            self.state_memory[agent_idx][index] = critic_sates[agent_idx]
            self.new_state_memory[agent_idx][index] = critic_new_states[agent_idx]
